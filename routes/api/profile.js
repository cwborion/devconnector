...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    if (err.kind == 'ObjectId') {
      return res.status(400).json({ msg: 'Profile nof found' });
    }
    res.status(500).send('Server Error');
  }
});
...
router.delete('/experience/:exp_id', auth, async (req, res) => {
  try {
    ...
    const removeIndex = profile.experience
      // Maps through experience array and returns all experience IDs
      .map(item => item.id)
      // Returns index of experience in array with ID provided from route '/experience/:exp_id'
      .indexOf(req.params.exp_id);

    // Removes 1 experience with index returned from above from given starting point in array "removeIndex"
